...
void writeU( double *u, double *r, int nr, int it, int writeProfile, double uRef, double Re, double ReTau)
{
 ...
 if ( 0 < writeProfile )                              // output file requested ?
 {
  ...
  if ( fileStream )                                                // file good?
  {
   ...
   for ( int j = 0; j < nr + 1; ++j )         // write radial profile (r = 0..R)
   {
    ...
    fileStream << setw(20) << u[j] / uRef * Re / ReTau;               // velocity
    fileStream << endl ;
               ...
               int nt, double uRef )
{
 ...
 if ( fileStream )                                                 // file good?
 {
  ...
  for ( int i = 0; i < nt; ++i )
  {
   ...
   fileStream << setw(23) << uMean[i]; //  / uRef;            // cross-sectional mean
   fileStream << setw(23) << uCenterLine[i]; // / uRef;               // center line
   fileStream << setw(23) << uGradWall[i];// / uRef;               // wall gradient
   fileStream << setw(23) << uTau[i];// / uRef;                // friction velocity
   fileStream << endl ;
